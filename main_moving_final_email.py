...
def save_data(H1_time, H2_time, H3_time, player_position_H1, player_position_H2, player_position_H3, cross_road_H1, 
    ...
    df_position_H1.to_csv('position_data_H1.csv', index=False)
    df_position_H2.to_csv('position_data_H2.csv', index=False)
    df_position_H3.to_csv('position_data_H3.csv', index=False)
    df_pedestrian_positions_H2.to_csv('pedestrian_positions_H2.csv', index=False)
    df_pedestrian_positions_H3.to_csv('pedestrian_positions_H3.csv', index=False)
    df_extra.to_csv('extra_data.csv', index=False)
    df_click_position_H1.to_csv('click_position_data_H1.csv', index=False)
    df_click_position_H2.to_csv('click_position_data_H2.csv', index=False)
    df_click_position_H3.to_csv('click_position_data_H3.csv', index=False)
...
instructions_text_1 = ['Thank you for taking part in this study!',
                       ...
                       '3. Data collected will be analysed to help understand how pedestrian route choices are affected by light and crowds,', 
                       'in order to improve pedestrian friendly spaces.',
                       '4. Data will only be recorded if you email the data files produced by the experiment to qv20601@bristol.ac.uk',
                       '5. Your email will be deleted once you have recieved a conformation email from the researcher and your email will not be stored.',
                       'It will not be possible to identify you from the data collected.',
                       '6. You understand that after the study, anonymised data may be made available as “open data”. ',
...
instructions_text_2 = ['Welcome to a simple route choice simulation!',
                        ...
                        'This means that the environment will move around you as you move, so at times it may look like you aren\'t moving when you are.',
                        '',
...
final_screen_text = ['Thank you for participating in the study!',
                     '',
                     'Nine files containing data collected during the simulation will be downloaded to your computer when this window is closed.',
                     'They should appear in the same folder as this simulation.',
                     'Please email all nine files to qv20601@bristol.ac.uk',
                     '',
